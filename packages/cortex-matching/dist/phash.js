...
exports.hammingToSimilarity = hammingToSimilarity;
...
function dct2d(pixels, n) {
    ...
    // Row-wise 1-D DCT
    const rowDct = new Array(n * n).fill(0);
...
async function computePhash(base64Image) {
    ...
    const { data: pixels } = await (0, sharp_1.default)(buffer)
...
function hammingDistance(hashA, hashB) {
    if (hashA.length !== hashB.length) {
        throw new Error(`pHash length mismatch: ${hashA.length} vs ${hashB.length}. Both must be 16-char hex strings.`);
    }
    let distance = 0;
